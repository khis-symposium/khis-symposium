...
//
// Local change: timeScale slowed down (0.86 -> 0.45) and the continuous
// time-based render loop is skipped under prefers-reduced-motion — pointer
// interaction still responds, but the ambient drift stops, matching the
// reduced-motion behavior used elsewhere on this site.
...
const UNIFORMS = {
  ...
  cursorRadius: 0.460,
  oklab: 0.0,
  // Slowed from the generated 0.86 per site request ("motion slightly slower").
  timeScale: 0.45,
}
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const timeAnimated = Math.abs(UNIFORMS.timeScale) > 0.0001 && !prefersReducedMotion
    ...
    function render(now: number) {
      ...
      resizeCanvas()
      // Non-null: `canvas`/`gl` were guarded at the top of the effect: TS
      // doesn't carry that narrowing into this nested `function render`
      // closure, but the guard already returned before render() could exist.
